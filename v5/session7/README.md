...
**This paper addresses Assignment Problem 4 ("A real Fourier alternative to Kronecker").** Design B (HRR/circular-convolution binding, §3.4/§4.4) is discussed only as spectral-domain background for Design A and as a secondary exploratory arm; its invertibility property is real and is measured (§18), but its Problem-5 consequence — eliminating the output head to reach 1M+ vocab — is deliberately out of scope for this submission.
...
**Scope note.** Circular-convolution binding is exactly invertible ($a \circledast b \circledast b^{-1} = a$, §3.4), and that invertibility is real, load-bearing machinery for a *different* assignment problem — Problem 5 asks for an invertible Kronecker specifically so the output head can be eliminated at 1M+ vocab, and HRR's unbinding property is exactly the mechanism such a construction would need. This paper does not draw that consequence: Design B is scoped here strictly as spectral-domain background and a secondary exploratory arm for **Problem 4** (§1), measured only for its own trade (crosstalk vs. $D$/$n$, §8.3/§18), not extended toward eliminating the output head or evaluated at 1M+ vocab scale.
...
>
> **Implementation note (added after an internal review caught a gap between this claim and the shipped code, and fixed before the run reported in §16 was retrained):** the mathematical claim above was always true of $\phi(p)$ in isolation, but the first cut of `ByteGridCodec` built its byte-value lookup buffer at a single shared width, `pos_dim`, for *both* codecs — so `FourierKroneckerEmbedding` was silently cropping tokens at `pos_dim` exactly like Kronecker, contradicting this section for the actual trained model even though the math it's based on was never wrong. The fix decouples the two: the codec now has a `byte_capacity` parameter (the real buffer width) that, for the Fourier/HRR position factors specifically, defaults to the longest token actually present in the vocabulary — so no real token is cropped — while Kronecker's `byte_capacity` stays hard-tied to `pos_dim`, unchanged, since its one-hot position table only has `pos_dim` rows by construction. §16 explains why this project's specific tokenizer/`pos_dim` choice still doesn't put visible pressure on the fix even with the code corrected, and §20 now scopes the follow-up run that would.
...
>
> This is now purely a data/tokenizer fact, not a code gap — see §6.1's implementation note. An earlier version of this section attributed the non-result to this cause alone while `ByteGridCodec` was *also* silently cropping the Fourier arm at `pos_dim` regardless of tokenizer choice, which would have meant that even a smaller `pos_dim` run wouldn't have exercised §6.1's claim either. That code path is fixed (§6.1), so the diagnosis above is now the complete explanation, and §20's follow-up recommendation is scoped accordingly.
...
- **`pos_dim=32` with a BPE tokenizer does not test the truncation-wall claim.** §16 is this paper's central limitation. The most valuable single follow-up is a repeat of this comparison at a small `pos_dim` (e.g. 8–16) and/or with longer average tokens, specifically to put real pressure on §6.1's claim. With the `byte_capacity` fix (§6.1) in place, this follow-up now genuinely exercises the claim it's meant to: at a small `pos_dim`, Kronecker's exact-collision rate should rise sharply once real tokens exceed `pos_dim` bytes, while Fourier's should stay low, since its buffer auto-sizes to the longest real token independent of `pos_dim`. Before this fix, the same follow-up would have moved *where* the (still-present) cropping happened without actually testing the claim — see §6.1's implementation note.
