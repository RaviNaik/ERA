...
Output: bpe_assignment/corpus/{lang}.faithful.md / .txt / .meta.json / .raw.html
...
USER_AGENT = "ERA V5 session2 tokenizer/1.0"
...
def main() -> int:
    print("Fetching Wikipedia pages and converting to faithful Markdown ...")
    for code, (name, title) in PAGES.items():
        print(f"  [{code}] {name}: {title} ...", end=" ", flush=True)
        meta = build_one(code, title)
        print(f"{meta['faithful_units']:,} faithful units, {meta['chars']:,} chars")
    print("\nDone. Corpus written to:", OUT)
    return 0
