/* ============================================================
   ...
   a verification pass run on a 4GB laptop GPU (RTX 500 Ada) at
   short budgets (220-300 steps) to confirm the mechanisms hold.
   PRELIMINARY — to be replaced once the real notebooks are run
   at full budget on the GPU server (see the banner in the hero).
   ============================================================ */
...
window.SESSION_DATA = {
  ...
  setup: {
    hardware: "Verification pass: RTX 500 Ada (4GB laptop GPU) · CUDA 12.4 · PyTorch 2.6.0",
    ...
    tracking: "Aim — every run logged: loss, lr, grad_norm, per-layer update/weight ratio",
    aimRuns: 57,
  },
  ...
  heroStats: [
    ...
    { val: "+0.08", lbl: "WSD beats cosine, step 300 (lower loss)", cls: "green" },
    { val: "≈2e-4", lbl: "predicted LR at width 4096 (low confidence)", cls: "rose" },
  ],
  ...
  tasks: [
    ...
    },
    {
      ...
      detail: "c(t) = √(1−β₂ᵗ) / (1−β₁ᵗ). Stays within 1% of 1 only after 76 steps (β2=0.95), 390 steps (β2=0.99), or 3,916 steps (β2=0.999). Replayed on a real weight's first 20 true gradients from the 10.77M model — the two trajectories differ by 77% of the weight's total movement after 20 steps.",
      verdict: "232 steps",
    },
    {
      n: 3, tag: "warmup", title: "Update/weight ratio × warmup",
      headline: "argmax(ρ) lands on the warmup length to within 1 step",
      detail: "ρ = ‖ΔW‖_RMS / ‖W‖_RMS, logged per layer every step across 300-step runs at warmup ∈ {0, 50, 200}. ρ settles at ≈1.0–1.4e-3 for every attention/MLP block — right on the folklore 1e-3 target. LayerNorm gains run ~35× colder. Skipping warmup cost 0.45 nats of final loss on this budget.",
      verdict: "ρ≈1e-3",
    },
    {
      n: 4, tag: "schedule", title: "Cosine vs WSD, judged at step 200",
      headline: "WSD ties cosine at step 200, wins by 0.08 nats at the full 300-step horizon",
      detail: "Same model, warmup 50, base_lr 1.5e-3. Frozen mid-plateau at step 200, cosine edges WSD by 0.016 (WSD hasn't started decaying yet). Re-planned to a 200-step budget, WSD beats cosine by 0.029. At the full 300 steps, WSD's longer high-LR plateau wins outright.",
      verdict: "keep WSD",
    },
    {
      n: 5, tag: "lrsweep", title: "LR sweep across width → predict width 4096",
      headline: "optimum flat 256→512 (~1.5e-3), drops sharply at 1024 (~5e-4)",
      detail: "Coarse 7-point grid × 3 widths, then a 2-seed 5-point refinement per basin (57 runs total). Power-law fit η*∝width^b gives b=−0.84 with a large residual (σ≈0.53) — the slope is carried almost entirely by the width-1024 point. Extrapolation: η*(4096) ≈ 1.8e-4, confidence LOW (factor of ~3).",
      verdict: "≈2e-4, LOW",
    },
  ],
  ...
  bias: {
    ...
    realWeight: {
      ...
      beta2: 0.99,
      rows: [
        { t: 1, c: 1.000, ratio: 1.001, gap: "1e-6" },
        { t: 5, c: 0.541, ratio: 0.541, gap: "2.0e-3" },
        { t: 10, c: 0.475, ratio: 0.475, gap: "4.5e-3" },
        { t: 20, c: 0.486, ratio: 0.486, gap: "6.9e-3" },
      ],
      gapPctOfMovement: 77,
    },
    step1Multiplier: [
  ...
  ratio: {
    layers: [
      ...
      { layer: "L0.mlp", r5: "5.3e-3", r50: "8.5e-3", r150: "5.9e-3", r299: "1.3e-3" },
      { layer: "L3.mlp", r5: "4.7e-3", r50: "6.5e-3", r150: "4.5e-3", r299: "1.3e-3" },
      { layer: "L5.mlp", r5: "4.8e-3", r50: "6.2e-3", r150: "4.4e-3", r299: "1.3e-3" },
    ...
    globalFinal: "7.7e-4",
    warmupRuns: [
      { warmup: 0, argmax: 0, peak: "3.7e-2", finalVal: 2.4405 },
      { warmup: 50, argmax: 49, peak: "4.6e-3", finalVal: 1.9915 },
      { warmup: 200, argmax: 199, peak: "5.4e-3", finalVal: 1.9981 },
    ],
    findings: [
      "Reported answer: the step at which warmup stops changing ρ IS the warmup length itself — warmup ∈ {0, 50, 200} stops changing ρ at step ≈ {0, 50, 200}. argmax(ρ) lands there to within 1 step.",
      "Per-layer ρ converges to 1.0–1.4e-3 for every attention/MLP block; depth barely matters (spread under 1.3×).",
      "Skipping warmup hurt a lot on this budget: warmup=0 finished at val 2.44 vs 1.99 for warmup∈{50,200} — grad-clip shock on steps 0–3 plus a lower average LR (cosine starts decaying immediately). Past 'enough' warmup, the exact value barely matters.",
    ],
  },
  ...
  schedules: {
    ...
    checkpoints: [
      ...
      { name: "WSD (300-plan), stop at 200", loss: 2.089, lr: "1.5e-3 (plateau)", tag: "" },
      { name: "WSD re-planned to a 200 budget", loss: 2.044, lr: "decayed 160→200", tag: "best" },
      { name: "cosine, full 300", loss: 1.921, lr: "1.5e-4", tag: "" },
      { name: "WSD, full 300", loss: 1.844, lr: "1.7e-4", tag: "best" },
    ],
    lrCurve: {
      steps: [0, 25, 50, 75, 100, 125, 150, 175, 200, 225, 250, 275, 299],
      cosine: [0.00003, 0.00098, 0.0015, 0.00144, 0.00126, 0.001, 0.00072, 0.00046, 0.00025, 0.0001, 0.00015, 0.00015, 0.00015],
      wsd:    [0.00003, 0.00098, 0.0015, 0.0015, 0.0015, 0.0015, 0.0015, 0.0015, 0.0015, 0.00113, 0.00063, 0.0002, 0.00017],
    },
    findings: [
      "Frozen mid-plan at step 200, cosine (2.073) beats plateau-WSD (2.089) by a thin 0.016 — the 'you stopped me mid-plateau' penalty.",
      "Told up front to stop at 200, WSD decays properly and its checkpoint (2.044) beats cosine-at-200 by 0.029 — higher average LR, still got its cooldown.",
      "At the full 300-step horizon WSD wins outright (1.844 vs 1.921, −0.076).",
      "Decision: keep the WSD full-300 checkpoint. WSD gives up essentially nothing versus cosine and removes the 'commit to the horizon before you start' constraint. Only if forced to stop at exactly step 200 with no re-plan would cosine win, and only by 0.016.",
    ],
  },
  ...
  lrsweep: {
    widths: [
      { width: 256, params: "4.82M", fitted: "1.5e-3", gridBest: "1.4e-3", loss: "2.40", spread: "±0.002" },
      { width: 512, params: "19.1M", fitted: "1.6e-3", gridBest: "2.0e-3", loss: "2.17–2.22", spread: "±0.03" },
      { width: 1024, params: "75.9M", fitted: "5.0e-4", gridBest: "4.3e-4", loss: "2.17", spread: "±0.004" },
    ],
    /* coarse loss-vs-lr curves for the SVG chart, log10(lr) on x */
    coarseGrid: [0.0003, 0.0006, 0.001, 0.002, 0.004, 0.008, 0.015],
    coarseLoss: {
      256:  [2.4595, 2.4323, 2.4115, 2.4149, 2.4292, 2.4771, 2.5665],
      512:  [2.4068, 2.3742, 2.3476, 2.2010, 2.4953, 2.5810, 2.7060],
      1024: [2.2573, 2.1919, 2.4769, 2.5110, 2.6154, 2.8341, 3.1373],
    },
    powerLaw: { b: -0.84, sigma: 0.53, pred4096: "1.8e-4", band: ["6e-5", "5e-4"], endpointOnly: "1.4e-4" },
    recommendation: { value: "2e-4", confirmSweep: ["1e-4", "2e-4", "4e-4"], confidence: "LOW" },
    findings: [
      "The optimum is flat from width 256→512 (~1.5e-3), then drops sharply at 1024 (~5e-4).",
      "Power-law fit η*∝width^b gives b=−0.84, with a large residual σ≈0.53 (log-LR) — the slope is carried almost entirely by the 1024 point. SP theory predicts b=−1 for matmul params; non-scaling embedding/LayerNorm params pull it toward 0.",
      "Extrapolation to width 4096: η*(4096) ≈ 1.8e-4 (endpoint-only bracket 1.4e-4; ±2σ band 6e-5–5e-4). The moving segment alone (512→1024) points as low as 5e-5.",
  ...
  figures: [
    ...
    { src: "../optimizer_experiments/assets/aim_runs_explorer.png", cap: "Aim Runs Explorer — all 57 verification-pass runs, with per-layer update_weight_ratio columns pulled in live." },
