...
// Wikipedia: 100,000 articles | Sangraha: 20,000 Indic docs
...
window.PIPELINE_DATA = {
  ...
  wikipedia: {
    ...
    stages: [
      ...
      { id:2, name:"Normalize",      icon:"✨",
        ...
        example:{ before:"The caf&eacute; served &lt;b&gt;coffee&lt;/b&gt; to [USER]: customers…", after:"The café served coffee to customers…", note:"HTML entities unescaped, BOM stripped, ghost tag removed" },
        color:"#8b5cf6"
      },
      ...
      { id:8, name:"Manifest",       icon:"📋",
        ...
        example:{ before:"(clean admitted text)", after:'{"shard_id":"shard_821a5c845bbf","source":"wikimedia/wikipedia","license_class":"CC-BY-SA-4.0","token_count":312,"status":"ADMITTED"}', note:"Deterministic shard_id from SHA-256[:12]. V4 used non-deterministic Spark SQL row counters." },
        color:"#64748b"
      }
    ]
  },
  ...
  strategies: [
    { id:1, name:"Extraction",     icon:"🔍", color:"#6366f1", gap:"V4 kept nav links and cookie banners as document body", fix:"trafilatura on raw WARC; strip {{templates}}, [[links]], ==Sections==, [refs]", drop:"Stubs <150 chars, empty after markup strip" },
    { id:2, name:"Normalization",  icon:"✨", color:"#8b5cf6", gap:"V4 had no clean_text() function — 46 garbage tokens baked in", fix:"NFC Unicode → HTML unescape → strip BOM/ZWSP/BIDI → PRESERVE ZWNJ/ZWJ → collapse whitespace", drop:"Docs <100 chars after normalization" },
    { id:3, name:"Language ID",    icon:"🌐", color:"#06b6d4", gap:"V4 trusted directory names (verified/asm/) without runtime validation; Python dict bug caused silent misrouting", fix:"fastText detection on every doc + script-histogram validation (Unicode codepoint block coverage)", drop:"Confidence <0.70, wrong-language docs, <30% expected script coverage" },
    { id:4, name:"Quality Filter", icon:"🧹", color:"#10b981", gap:"V4 English-heavy proxy classifier penalised Indic scripts; allowed bad noise via always-on bypass", fix:"6 Gopher/C4 heuristic rules (word-len, symbol-ratio, line-len, punct-end, stopword, dup-lines) + FineWeb-Edu edu score ≥1.5; Indic-aware thresholds", drop:"2+ heuristic failures or edu_score <1.5" },
    { id:5, name:"Deduplication",  icon:"🔁", color:"#f59e0b", gap:"V4 only did local per-source dedup — cross-shard duplicates leaked, wasting compute", fix:"Pass 1: SHA-256 exact hash. Pass 2: MinHash LSH (128 perms, shingle k=5, Jaccard ≥0.80)", drop:"Exact SHA-256 matches, near-dups with Jaccard ≥0.80" },
    { id:6, name:"PII Scrub",      icon:"🔒", color:"#ef4444", gap:"V4 lacked PII scrubbing for Indic pipelines", fix:"Regex: email, IPv4, phone, AADHAAR, PAN. Typed placeholder [EMAIL_REDACTED] not empty string", drop:"Docs <50 chars after scrubbing (rare)" },
    { id:7, name:"Decontamination",icon:"🛡", color:"#f97316", gap:"V4 no active firewall — Band B2 deleted after 18.7% collision rate discovered post-hoc", fix:"n-gram fingerprint (n=8) + MCQ pattern scan against MMLU/GSM8K/HumanEval. 3-tier firewall.", drop:"Benchmark overlap or MCQ format detected" },
    { id:8, name:"Manifest",       icon:"📋", color:"#64748b", gap:"V4 used non-deterministic Spark SQL row counters; words×1.3 token estimate → 10x undercount for Indic", fix:"shard_id = sha256[:12] (content-addressed). Real BPE-aware token counting.", drop:"None — stamps provenance on all admitted docs" },
