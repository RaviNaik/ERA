...
> — runs top to bottom, every experiment done **by hand in the cells**.
> `S10_SMOKE=1` gives a ~2-minute CPU pass; the default config is a small-GPU proxy.
...
├── session10_training_loop.ipynb   # tasks 1-6, step by step
├── src/trainloop/
...
The notebook walks the forward pass **by hand**, one sub-module at a time
(`net.transformer.wte(x)`, `blk.attn.c_attn(xn)`, …), un-rolling attention
explicitly so the `[B, H, T, T]` score tensor is visible, then loops the
parameters and — after `backward()` — every `.grad`. Three tables:
...
The notebook picks `transformer.h.0.mlp.c_fc.weight[0,0]` on an fp64 `deepcopy`
with dropout off, and compares:
...
**The static case** (class-notes numbers)
...
**In a real loop** — the notebook defines `accumulate_correct` and
`accumulate_wrong` inline, then trains two copies of the same initial model, fed
the **same** unequal-length micro-batches every step (token counts
`(48, 48, 12)`), one under each rule:
...
The notebook loop logs, every step: raw loss, an EMA-smoothed loss, and the
global grad norm `sqrt(Σ gᵢ²)` (computed inline, before the optimizer step). At
one step it feeds a single poisoned batch (targets replaced with noise). That
batch has a large gradient → the norm spikes **on that step**. The optimizer
takes one bad step, and the loss on the *following* normal batches degrades.
...
The notebook spells out `flops_per_token = 6N + attn`, times a real forward +
backward + optimizer loop, counts the tokens through it, and divides by the
vendor dense peak (`S10_PEAK_TFLOPS` overrides the small built-in lookup).

| quantity | smoke run (CPU) |
| --- | --- |
| flops/token (`6N` + attn) | 721,152 |
| tokens/sec | ~145,000 |
| achieved | ~0.10 TFLOP/s |
| peak (CPU placeholder) | 0.5 TFLOP/s |
| **MFU** | **~21%** (meaningless — the peak is a placeholder; the number matters on a real GPU) |
...
(The notebook derives fp32/bf16 straight from the IEEE bit patterns via `struct`
/ numpy, and rounds the fp8 E4M3 mantissa by hand — round-to-nearest-ties-to-even
on 3 bits, with carry into the exponent and the 448 clamp.)
