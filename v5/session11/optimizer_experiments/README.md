...
> **Status.** The code (model / hand-optimizer / schedules / trainer / Aim
> tracking) is complete and passes `scripts/smoke_test.py` (every code path
> exercised end-to-end on a tiny model in seconds). The **notebooks ship
> unexecuted** — `notebooks/*.ipynb` are the source of truth, generated by
> `scripts/build_notebooks.py`. The numbers, tables and figures in this README
> (including `assets/*.png`) are from a **verification pass** run on a 4 GB
> laptop GPU (RTX 500 Ada) at deliberately short budgets (220–300 steps) to
> confirm the mechanisms and conclusions hold; that run's `.aim/` and
> `results/runs.jsonl` were not kept in this checkout. **Run
> `uv run python scripts/run_all.py` to execute the real notebooks** — bump
> `total_steps` / `batch_size` / the LR grids first if you want a bigger budget
> on better hardware; the *mechanisms* below should replicate, the *exact loss
> values* will shift.
...
| 4 | Cosine vs WSD, 300 steps, judge at 200 | [`04_cosine_vs_wsd.ipynb`](notebooks/04_cosine_vs_wsd.ipynb) | **keep WSD** — within 0.02 at 200, wins by **0.08** at 300 |
| 5 | LR sweep at width 256/512/1024 → predict width 4096 | [`05_lr_sweep_width.ipynb`](notebooks/05_lr_sweep_width.ipynb) | η\*(4096) ≈ **2e-4**, **low confidence** (factor ~3) |
...
│   └── utils.py
├── notebooks/        # 01 … 05, one per task (source cells; run to populate outputs)
├── scripts/
...
└── assets/                  # figures embedded below (from the verification pass)

# created when you run the notebooks — not part of this checkout:
#   .aim/                    Aim repo (git-ignored)
#   results/runs.jsonl       every training run, durable + machine-independent
#   results/runs_summary.csv flattened run table
```
...
uv run python scripts/run_all.py            # the real thing: all 5 notebooks
uv run python scripts/run_all.py 1 2 3      # or just a subset of notebooks
uv run python scripts/aim_summary.py        # results/runs.jsonl -> summary table
uv run aim up                               # browse every run at localhost:43800
```

`run_all.py` (re)builds the notebooks from `scripts/build_notebooks.py`, executes
them into a fresh Aim repo, and prints the run table. On the 4 GB laptop GPU used
for verification this took ~55 min end-to-end; scale it up on better hardware by
raising `total_steps` / `batch_size` (and the LR grids in task 5) in the relevant
notebook config cells first — everything else (Aim tracking, plots, findings
scaffolding) needs no changes.

### Aim tracking

Every training run goes through `s11.trainer.train()` → one Aim run
(experiments: `task3_warmup`, `task4_schedules`, `task5_lr_width`). Tracked per
run:
...
> **Note observed during verification.** Aim's RocksDB *run-attribute* store
> (the params panel) did not survive process exit on this machine's overlay
> filesystem — metrics were fine, params showed empty in the UI. `trainer.train`
> works around this: run names encode the key settings
> (`w512_lr2.0e-03_s7_refine`), and it additionally appends a flat JSON record
> (full hyper-parameters + loss curve) to `results/runs.jsonl` on every run —
> check that file's behavior first if params look empty in your Aim UI too.

Screenshots below are from the verification run's Aim UI (57 runs, before that
`.aim/` repo was cleared) — your own `run_all.py` run will produce the same
views against its own data:

**Runs Explorer** — all 57 runs, with the per-layer `update_weight_ratio` columns
(`layer="L4.mlp"`, `layer="L4.attn"`, …) pulled in:
...
then climbs back — so "stops mattering" means *enters the band and stays*:
...
The first 20 true gradients this weight receives, replayed with correction on/off:

| t | c(t) predicted | &#124;step ratio&#124; measured | &#124;w_BC − w_noBC&#124; |
|--:|--:|--:|--:|
| 1  | 1.000 | 1.001 | 1e-6 |
| 5  | 0.541 | 0.541 | 2.0e-3 |
| 10 | 0.475 | 0.475 | 4.5e-3 |
| 20 | 0.486 | 0.486 | 6.9e-3 |

After 20 steps the two weights differ by **6.9e-3 — about 77% of how far the
weight has moved at all** — because at β₂ = 0.99 the correction factor is still
`c(20) = 0.49`, nowhere near 1.
...
| L0.mlp  | 5.3e-3 | 8.5e-3 | 5.9e-3 | 1.3e-3 |
| L3.mlp  | 4.7e-3 | 6.5e-3 | 4.5e-3 | 1.3e-3 |
| L5.mlp  | 4.8e-3 | 6.2e-3 | 4.4e-3 | 1.3e-3 |
...
gains run **~35× colder** than the matmul blocks.
...
| warmup | argmax ρ (step) | ρ peak | warmup length |
|--:|--:|--:|--:|
| 0   | **0**   | 3.7e-2 | 0 |
| 50  | **49**  | 4.6e-3 | 50 |
| 200 | **199** | 5.4e-3 | 200 |

**argmax(ρ) = the warmup length**, to within 1 step. Before it, warmup pushes ρ
up along the ramp; after it, cosine decay pulls ρ down. Dividing the LR out
(`ρ/η`) leaves only a short opening transient (grad-clip firing on steps 0–3,
gnorm 16 → <1; `v̂` still warming) that lasts ~15–55 steps and is **identical
across warmups** — not a warmup effect.
...
- **Reported step: the warmup length itself.** warmup ∈ {0, 50, 200} stops
  changing ρ at step ≈ {0, 50, 200}.
- Per-layer ρ converges to **1.0–1.4e-3** for every attention/MLP block;
  depth barely matters (spread < 1.3×).
- **Skipping warmup hurt a lot here:** `warmup=0` finished at val **2.44** vs
  **1.99** for `warmup ∈ {50, 200}` — grad-clip shock on steps 0–3 *and* a lower
  average LR (cosine starts decaying from step 0). Past "enough" warmup, the
  exact value barely matters (`50` and `200` finish within 0.005).
...
| WSD (300-plan), stop at 200 | 2.089 | 1.5e-3 (still on plateau) |
| **WSD re-planned to a 200 budget** | **2.044** | decayed 160→200 |
| cosine, full 300 | 1.921 | |
| **WSD, full 300** | **1.844** | |
...
- Frozen mid-plan at step 200, cosine (2.073) beats plateau-WSD (2.089) by a thin
  **0.016** — that's the "you stopped me mid-plateau" penalty.
- Told up front to stop at 200, WSD decays properly and its checkpoint (**2.044**)
  *beats* cosine-at-200 by 0.029 — higher average LR, still got its cooldown.
...
`n_head=8`, cosine, warmup 40, `wd=0.1`, `β₂=0.99`, batch 16.

### The three minima (2-seed refinement)

| width | params | fitted η\* | grid-best η\* | val loss @ 220 | 2-seed spread near η\* |
|---|--:|--:|--:|--:|--:|
| 256  | 4.82 M  | **1.5e-3** | 1.4e-3 | 2.40 | ±0.002 |
| 512  | 19.1 M  | **1.6e-3** | 2.0e-3 | 2.17–2.22 | **±0.03** |
| 1024 | 75.9 M  | **5.0e-4** | 4.3e-4 | 2.17 | ±0.004 |
...
- The optimum is **flat from width 256 → 512** (~`1.5e-3`), then **drops sharply
  at 1024** (~`5e-4`).
- Power-law fit `η* ∝ width^b` → **b = −0.84**, but with a **large residual
  σ ≈ 0.53** (in log-LR) — because 256 and 512 barely differ, the slope is
...
- **Extrapolation to width 4096: η\*(4096) ≈ `1.8e-4`** (endpoint-only bracket
  `1.4e-4`; ±2σ band `6e-5 … 5e-4`). The moving segment alone (512→1024) points
  as low as `5e-5`.
...
**Confidence: LOW** — factor of ~3. Reasons: (1) only 3 width points, σ≈0.5,
...
flowchart LR
    ...
    D --> E["power law lr* ~ width^b   (b = -0.84, sigma = 0.53)"]
    E --> F["extrapolate to lr*(4096) ~ 2e-4   (LOW confidence)"]
```
...
Shakespeare. Tracking: Aim. Env: `uv`, PyTorch 2.6 + CUDA 12.4, RTX 500 Ada (4 GB).</sub>
